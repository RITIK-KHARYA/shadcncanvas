...
type MarketShareDatum = { product: string; value: number; share?: string };

export function EChartsMarketSharePieChart({ data }: { data?: MarketShareDatum[] }) {
  const rows = (data && data.length ? data : chartData).map((row) => ({
    ...row,
    share: row.share ?? `${row.value}%`,
  }));
