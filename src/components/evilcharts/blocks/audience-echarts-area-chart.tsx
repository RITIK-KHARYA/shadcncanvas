...
type AudienceDatum = { month: string; listeners: number };

export function EChartsAudienceAreaChart({ data }: { data?: AudienceDatum[] }) {
  const rows = data && data.length ? data : chartData;
  const TOTAL = rows.reduce((sum, { listeners }) => sum + listeners, 0);
  return (
