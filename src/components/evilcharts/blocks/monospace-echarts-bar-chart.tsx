...
type MonospaceBarDatum = { month: string; sales: number };

export function EChartsMonospaceBarChart({ data }: { data?: MonospaceBarDatum[] }) {
  const rows = data && data.length ? data : chartData;
  const TOTAL = rows.reduce((sum, { sales }) => sum + sales, 0);
  const TOP = rows.reduce((max, row) => (row.sales > max.sales ? row : max), rows[0]);
  return (
