...
type ShipmentsLineDatum = { slot: string; current: number; previous: number };

export function EChartsShipmentsLineChart({ data }: { data?: ShipmentsLineDatum[] }) {
  const rows = data && data.length ? data : chartData;
  const TOTAL = rows.reduce((sum, { current }) => sum + current, 0);
  return (
