...
export function EChartsCacheTiersRadialChart({
  ...
  const memoryCount = hits ?? TIERS[0].count;
  const otherCount = Math.max(0, total - memoryCount);
  const otherRatio = TIERS.slice(1).reduce((sum, t) => sum + t.count, 0) || 1;
  const scaledTiers = [
    { ...TIERS[0], count: memoryCount },
    ...TIERS.slice(1).map((t) => ({
      ...t,
      count: (t.count / otherRatio) * otherCount,
    })),
  ];
  const scaledData = [...scaledTiers]
    .reverse()
    .map(({ name, count: c }) => ({
      name,
  ...
  return (
    <div className="flex h-full w-full flex-col gap-2 p-3 sm:gap-3 sm:p-4">
      ...
      <div className="grid shrink-0 grid-cols-2 gap-x-4 gap-y-2 border-t pt-2 sm:grid-cols-4 sm:gap-y-3 sm:pt-3">
        {scaledTiers.map(({ name, label, count: hits, swatch }) => (
          <div key={name} className="flex flex-col gap-1">
