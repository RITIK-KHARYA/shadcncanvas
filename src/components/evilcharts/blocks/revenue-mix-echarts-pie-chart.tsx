...
export function EChartsRevenueMixPieChart({ data }: { data?: RevenueMixDatum[] }) {
  const rows = (data && data.length ? data : chartData).map((row) => ({
    ...row,
    label: row.label ?? (chartData.find((c) => c.channel === row.channel)?.label ?? row.channel),
    swatch:
      row.swatch ?? (chartData.find((c) => c.channel === row.channel)?.swatch ?? "bg-muted"),
  }));
  return (
    <div className="flex h-full w-full items-center gap-3 p-4 sm:gap-6">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col justify-center">
        {rows.map(({ channel, label, value, swatch }) => (
