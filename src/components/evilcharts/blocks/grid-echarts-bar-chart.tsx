...
type GridBarDatum = { hour: string; sessions: number };

export function EChartsGridBarChart({ data }: { data?: GridBarDatum[] }) {
  const rows = data && data.length ? data : chartData;
  const TOTAL = rows.reduce((sum, { sessions }) => sum + sessions, 0);
