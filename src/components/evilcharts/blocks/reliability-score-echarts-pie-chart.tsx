...
export function EChartsReliabilityScorePieChart({ data }: { data?: ReliabilityDatum[] }) {
  const rows = (data && data.length ? data : chartData).map((row) => {
    const base = chartData.find((c) => c.band === row.band);
    return {
      ...row,
      label: row.label ?? base?.label ?? row.band,
      from: row.from ?? base?.from ?? 0,
      bar: row.bar ?? base?.bar ?? "bg-muted",
    };
  }) as typeof chartData;
  const BAND = [...rows].reverse().find(({ from }) => SCORE >= from) ?? rows[0];
